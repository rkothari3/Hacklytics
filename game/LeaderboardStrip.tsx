      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View style={styles.row}>
          {sorted.map((entry, i) => (
            <View
              key={entry.name}
              style={[styles.entry, entry.isCurrentUser && styles.entryHighlight]}
            >
              <Text style={styles.rank}>{i + 1}.</Text>
              <Text style={[styles.name, entry.isCurrentUser && styles.nameHighlight]}>
                {entry.isCurrentUser ? 'YOU' : entry.name}
              </Text>
              <Text style={styles.tickets}>🎫×{entry.goldenTickets}</Text>
            </View>
          ))}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  entryHighlight: {
    backgroundColor: COLORS.goldDark + '33',
    borderRadius: 6,
    paddingHorizontal: 4,
  },
  rank: {
